...
def post_command(base_url: str, payload: dict) -> bool:
    r = requests.post(f"{base_url.rstrip('/')}/api/commands", json=payload, timeout=10)
...
def post_image(base_url: str, path: str, name: str | None) -> bool:
...
def post_audio(base_url: str, path: str, name: str | None) -> bool:
    base_url = base_url.rstrip("/")
    if not os.path.isfile(path):
        print(f"File not found: {path}", file=sys.stderr)
        return False
...
def main():
    ...
    if args.message:
        ok = post_command(args.url, {"type": "print", "data": args.message, "source": args.source})
    elif args.image:
        ok = post_image(args.url, args.image, args.name)
    elif args.audio:
        ok = post_audio(args.url, args.audio, args.name)
    else:
        ap.print_help()
        return 1
